...
const taskStatus = ["To Do", "Work In Progress", "Under Review", "Complected"];
...
const Task = ({ task }: TaskProps) => {
  ...
  const taskTagsSplict = task.tags ? task.tags.split(",") : [];
  ...
  const PriorityTag = ({ priority }: { priority: TaskType["priority"] }) => {
    if (!priority) {
      return null; // Return null if priority is undefined, avoiding comparisons
    }
            ...
            <div className="flex -space-x-[6px] overflow-hidden">
              {task.assignee && task.assignee.profilePictureUrl && (
                <Image
                  ...
                  src={`/${task.assignee.profilePictureUrl}`} // Safely access the profilePictureUrl
                  alt={task.assignee.username || "Assignee"} // Fallback to "Assignee" if username is undefined
                  width={30}
                  height={30}
              ...
              {task.author && task.author.profilePictureUrl && (
                <Image
                  ...
                  src={`/${task.author.profilePictureUrl}`} // Safely access the profilePictureUrl
                  alt={task.author.username || "Author"} // Fallback to "Author" if username is undefined
                  width={30}
                  height={30}
                  className="h-8 w-8 rounded-full border-2 border-white object-cover dark:border-dark-secondary"
                />
